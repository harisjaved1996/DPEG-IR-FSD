...
3. **Review templates and assets**
   ...
   - For any test class work, always read and use `sf-apex-test` skill
...
5. **Generate test classes** -- Load the skill `sf-apex-test` to create `{ClassName}Test.cls` and `{ClassName}Test.cls-meta.xml`.  Apex tests are always required to be generated to deploy. No test file creation or edits can occur without loading the  `sf-apex-test` skill to generate tests.
...
7. **Execute Apex tests**
   ...
   - Delegate all test generation/fixes/coverage work to `sf-apex-test`; iterate until the tests pass.
...
- `{ClassName}Test.cls` (generated via `sf-apex-test` skill)
- `{ClassName}Test.cls-meta.xml` (generated via `sf-apex-test` skill)
...
| Need | Delegate to |
|---|---|
| Apex tests / fix failures | `sf-apex-test` skill |
...
This skill handles production `.cls`/`.trigger`/`.apex` issues only: compile/parse failures, deployment dependency errors, runtime governor-limit failures. For test execution, assertions, coverage, or `sf apex run test` failures, delegate to `sf-apex-test`.
