...
metadata:
  version: "1.0"
  related-skills: sf-custom-object, sf-custom-field, sf-custom-tab, sf-flexipage, sf-custom-application, sf-flow, sf-validation-rule, sf-list-view, sf-permission-set
---
...
- User only needs to create or configure an app container (grouping existing tabs) without other metadata; use `sf-custom-application` instead
...
|---------------|------------|-------------|------------|
| **Custom Object** | `sf-custom-object` | `salesforce-api-context` | MUST load skill AND call API context |
| **Custom Field** | `sf-custom-field` | `salesforce-api-context` | MUST load skill AND call API context |
| **Custom Tab** | `sf-custom-tab` | `salesforce-api-context` | MUST load skill AND call API context |
| **FlexiPage** | `sf-flexipage` | `salesforce-api-context` | MUST load skill AND call API context |
| **Custom Application** | `sf-custom-application` | `salesforce-api-context` | MUST load skill AND call API context |
| **List View** | `sf-list-view` | `salesforce-api-context` | MUST load skill AND call API context (if requested) |
| **Validation Rule** | `sf-validation-rule` | `salesforce-api-context` | MUST load skill AND call API context (if requested) |
| **Flow** | `sf-flow` | `metadata-experts` pipeline | MUST load skill AND run pipeline. **Exempt from `salesforce-api-context`**. |
| **Permission Set** | `sf-permission-set` | `salesforce-api-context` | MUST load skill AND call API context |
...
1. `sf-custom-object` - once, with all objects
2. `sf-custom-field` - once, with all fields (including Master-Detail, Lookup, Roll-up Summary)
...
1. `sf-validation-rule` - once, if validation requirements mentioned
2. `sf-flow` - once, if automation/workflow requirements mentioned
...
1. `sf-list-view` - once, for filtered record views (if requested)
2. `sf-custom-tab` - once, with all object tabs
3. `sf-flexipage` - once, with all record/home/app pages
...
1. `sf-custom-application` - once, to create the Lightning App container
...
1. `sf-permission-set` - once, with all permission sets and access to:
...
- CustomObject: load sf-custom-object + call salesforce-api-context
- CustomField: load sf-custom-field + call salesforce-api-context
- ValidationRule: load sf-validation-rule + call salesforce-api-context (if requested)
- Flow: load sf-flow + run metadata-experts pipeline (if requested)
- ListView: load sf-list-view + call salesforce-api-context (if requested)
- CustomTab: load sf-custom-tab + call salesforce-api-context
- FlexiPage: load sf-flexipage + call salesforce-api-context
- CustomApplication: load sf-custom-application + call salesforce-api-context
- PermissionSet: load sf-permission-set + call salesforce-api-context
...
**1. Custom Objects**
- ① Load skill: Read `sf-custom-object` SKILL.md
...
**2. Custom Fields**
- ① Load skill: Read `sf-custom-field` SKILL.md
...
- ① Load skill: Read `sf-validation-rule` SKILL.md
...
- ① Load skill: Read `sf-flow` SKILL.md
...
- ① Load skill: Read `sf-list-view` SKILL.md
...
**6. Custom Tabs**
- ① Load skill: Read `sf-custom-tab` SKILL.md
...
**7. FlexiPages**
- ① Load skill: Read `sf-flexipage` SKILL.md
...
- ① Load skill: Read `sf-custom-application` SKILL.md
...
**9. Permission Sets**
- ① Load skill: Read `sf-permission-set` SKILL.md
